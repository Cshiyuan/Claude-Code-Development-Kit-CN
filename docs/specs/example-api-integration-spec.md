...
本文档概述了与外部服务 API 的集成。这作为记录 API 集成规格的模板。
...
### 错误处理流程
```
API 错误 → 重试逻辑 → 回退策略 → 错误日志记录 → 客户端错误响应
```
    ...
    async def authenticate(self) -> None:
        ...
        async with self.session.post(auth_url, data=auth_data) as response:
            if response.status == 200:
                ...
                raise APIAuthenticationError("外部 API 身份验证失败")
```
...
async def process_data(
    ...
    raise ExternalAPIError("超过最大重试尝试次数")
```
...
def map_api_error(status_code: int, response_data: Dict[str, Any]) -> ExternalAPIError:
    """将 API 错误响应映射到自定义异常"""
    error_mapping = {
        400: ("错误请求", ExternalAPIError),
        401: ("未经授权", APIAuthenticationError),
        429: ("超出速率限制", APIRateLimitError),
        500: ("内部服务器错误", ExternalAPIError),
        503: ("服务不可用", ExternalAPIError)
    }
...
*此 API 集成规格模板提供了记录外部服务集成的全面方法。根据你的具体 API 要求和集成需求进行自定义。*
