...
**第 1 层(基础层)**: 稳定的、系统级文档,很少变更 - 架构原则、技术决策、跨组件模式和核心开发协议。

**第 2 层(组件层)**: 主要组件的架构章程 - 高层设计原则、集成模式和组件级约定,不包含功能特定细节。

**第 3 层(功能层)**: 与代码共置的细粒度文档 - 具体的实现模式、技术细节和随功能演进的本地架构决策。
...
- **[任务管理](/docs/ai-context/handoff.md)** - *会话连续性。* 当前任务、文档系统进度和下一会话目标
...
与代码共置的细粒度 CONTEXT.md 文件,以最小化级联效应:
...
- **[身份验证](/backend/src/auth/CONTEXT.md)** - *认证模式。* 身份验证流程、授权规则、会话管理和安全
- **[集成](/backend/src/integrations/CONTEXT.md)** - *外部服务。* 第三方 API 客户端、webhook 处理器和服务适配器
...
- **[路由](/frontend/src/routes/CONTEXT.md)** - *导航模式。* 路由定义、守卫、懒加载和深度链接
- **[工具](/frontend/src/utils/CONTEXT.md)** - *辅助函数。* 格式化器、验证器、转换器和常用工具

### 共享功能文档
- **[通用类型](/shared/src/types/CONTEXT.md)** - *类型定义。* 共享接口、枚举和类型工具
- **[验证规则](/shared/src/validation/CONTEXT.md)** - *验证模式。* 模式定义、自定义验证器和错误消息
- **[常量](/shared/src/constants/CONTEXT.md)** - *共享常量。* 配置值、枚举和魔术数字
- **[工具](/shared/src/utils/CONTEXT.md)** - *共享工具。* 跨平台辅助函数、格式化器和常用功能
...
*此文档架构模板应根据你项目的实际结构和组件进行自定义。根据你的架构添加或删除部分。*
